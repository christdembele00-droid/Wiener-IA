...
function wrappedExpress(...args) {
  ...
  app.get = function patchedGet(route, ...handlers) {
    if (route === "/api/brain/status") return originalGet(route, (_req, res) => res.json({ ok: true, version: BRAIN_VERSION, ...capabilities.status(), orchestration: true, automaticGrounding: true, conversationStore: true, fastReflection: true, reflectionBudgetMs: brain.REFLECTION_MAX_MS }));
    if (route === "/api/conversations") return originalGet(route, (_req, res) => res.json({ ok: true, conversations: loadConversations().map(({ messages, ...meta }) => meta) }));
    if (route === "/api/conversations/:id") return originalGet(route, (req, res) => { const item = loadConversations().find(x => x.id === req.params.id); if (!item) return res.status(404).json({ error: "Conversation introuvable." }); res.json({ ok: true, conversation: item }); });
    return originalGet(route, ...handlers);
  };

  app.post = function patchedPost(route, ...handlers) {
    if (route === "/api/brain/analyze") return originalPost(route, (req, res) => { const query = typeof req.body?.query === "string" ? req.body.query.trim() : ""; if (!query) return res.status(400).json({ error: "Requête vide." }); const reflection = brain.reflect(query); res.json({ ok: true, query, state: reflection.state, plan: reflection.plan, reflection: { elapsedMs: reflection.elapsedMs, budgetMs: reflection.budgetMs, withinBudget: reflection.withinBudget, mode: reflection.mode }, instruction: brain.buildInstruction(query, reflection.state) }); });
    if (route === "/api/brain/verify") return originalPost(route, (req, res) => { const query = typeof req.body?.query === "string" ? req.body.query : ""; const answer = typeof req.body?.answer === "string" ? req.body.answer : ""; const state = brain.classify(query); res.json({ ok: true, verification: brain.verifyResponse(answer, state) }); });
    if (route === "/api/conversations") return originalPost(route, (req, res) => { const items = loadConversations(); const now = new Date().toISOString(); const item = { id: conversationId(), title: cleanTitle(req.body?.title), messages: Array.isArray(req.body?.messages) ? req.body.messages.slice(-200) : [], createdAt: now, updatedAt: now, archived: false, pinned: false, temporary: Boolean(req.body?.temporary), folder: String(req.body?.folder || "").slice(0, 80) }; items.push(item); saveConversations(items); res.status(201).json({ ok: true, conversation: item }); });
    return originalPost(route, ...handlers);
  };

  app.patch = function patchedPatch(route, ...handlers) {
    if (route === "/api/conversations/:id") return originalPatch(route, (req, res) => { const items = loadConversations(); const item = items.find(x => x.id === req.params.id); if (!item) return res.status(404).json({ error: "Conversation introuvable." }); if (req.body?.title !== undefined) item.title = cleanTitle(req.body.title); if (Array.isArray(req.body?.messages)) item.messages = req.body.messages.slice(-200); if (req.body?.archived !== undefined) item.archived = Boolean(req.body.archived); if (req.body?.pinned !== undefined) item.pinned = Boolean(req.body.pinned); if (req.body?.temporary !== undefined) item.temporary = Boolean(req.body.temporary); if (req.body?.folder !== undefined) item.folder = String(req.body.folder || "").slice(0, 80); item.updatedAt = new Date().toISOString(); saveConversations(items); res.json({ ok: true, conversation: item }); });
    return originalPatch(route, ...handlers);
  };

  app.delete = function patchedDelete(route, ...handlers) {
    if (route === "/api/conversations/:id") return originalDelete(route, (req, res) => { const items = loadConversations(); const next = items.filter(x => x.id !== req.params.id); if (next.length === items.length) return res.status(404).json({ error: "Conversation introuvable." }); saveConversations(next); res.json({ ok: true }); });
    if (route === "/api/conversations") return originalDelete(route, (_req, res) => { saveConversations([]); res.json({ ok: true }); });
    return originalDelete(route, ...handlers);
  };

  const brainMiddleware = (req, _res, next) => {
    try {
      const query = typeof req.body?.message === "string" ? req.body.message : typeof req.body?.text === "string" ? req.body.text : typeof req.body?.question === "string" ? req.body.question : Array.isArray(req.body?.messages) ? req.body.messages.slice(-10).map(m => m?.content || "").join(" ") : "";
      if (query) {
        const reflection = brain.reflect(query);
