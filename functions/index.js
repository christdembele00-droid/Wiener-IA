...
function configure(){
  ...
  const webApiKey=process.env.WIENER_FIREBASE_WEB_API_KEY||"";
...
app.use((_,__,next)=>{configure();next()});
...
async function firebaseAuth(action,email,password){const key=process.env.WIENER_FIREBASE_WEB_API_KEY||"";if(!key)throw Object.assign(Error("WIENER_FIREBASE_WEB_API_KEY est manquante."),{status:503});const r=await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:${action}?key=${encodeURIComponent(key)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password,returnSecureToken:true})});const d=await r.json();if(!r.ok)throw Object.assign(Error(d?.error?.message||"Authentification impossible."),{status:r.status});return d}
...
app.get("/api/health",(_,res)=>res.json({ok:true,service:"Wiener IA",serverless:true,geminiConfigured:Boolean(ai),firebaseConfigured:true,cloudinaryConfigured:Boolean(cloudinary.config().cloud_name),authConfigured:Boolean(process.env.WIENER_FIREBASE_WEB_API_KEY),textModel:TEXT_MODEL,time:new Date().toISOString()}));
app.get("/health",(_,res)=>res.json({ok:true,serverless:true,service:"Wiener IA",geminiConfigured:Boolean(ai),firebaseConfigured:true,cloudinaryConfigured:Boolean(cloudinary.config().cloud_name),authConfigured:Boolean(process.env.WIENER_FIREBASE_WEB_API_KEY),textModel:TEXT_MODEL,time:new Date().toISOString()}));
