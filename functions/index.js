...
app.get("/api/health",(_,res)=>res.json({ok:true,service:"Wiener IA",serverless:true,geminiConfigured:Boolean(ai),firebaseConfigured:true,cloudinaryConfigured:Boolean(cloudinary.config().cloud_name),authConfigured:Boolean(process.env.FIREBASE_WEB_API_KEY),textModel:TEXT_MODEL,time:new Date().toISOString()}));
app.get("/health",(_,res)=>res.json({ok:true,serverless:true,service:"Wiener IA",geminiConfigured:Boolean(ai),firebaseConfigured:true,cloudinaryConfigured:Boolean(cloudinary.config().cloud_name),authConfigured:Boolean(process.env.FIREBASE_WEB_API_KEY),textModel:TEXT_MODEL,time:new Date().toISOString()}));
...
exports.api=onRequest({region:"us-central1",timeoutSeconds:120,memory:"1GiB",maxInstances:10,secrets:["GEMINI_API_KEY","CLOUDINARY_URL","FIREBASE_WEB_API_KEY"]},app);
