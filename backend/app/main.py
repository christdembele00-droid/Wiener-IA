...
app = FastAPI(title="Wiener-IA API", version="5.0.0")
...
async def health() -> dict[str, object]:
    return {"ok": True, "service": "Wiener-IA", "version": "5.0.0"}
...
async def status() -> dict[str, object]:
    return {
        ...
        "learning": cycle.learning.snapshot(),
        "evolution": {
            "generation": cycle.evolution.generation,
            "strategy_preferences": cycle.evolution.strategy_preferences,
        },
        "pipeline": ["perception", "context", "memory", "reasoning", "planning", "selection", "action", "reflection", "learning", "evolution", "inheritance"],
    }
...
async def evolution() -> dict[str, object]:
    return {
        "generation": cycle.evolution.generation,
        "preferences": cycle.evolution.strategy_preferences,
        "genome": cycle.inheritance.build(
            {"generation": cycle.evolution.generation, "preferences": cycle.evolution.strategy_preferences},
            cycle.state.history,
        ).export(),
    }
...
async def metrics() -> dict[str, object]:
    return {
        "cycles": cycle.internal.cycle_count,
        "memory_items": cycle.memory.count(),
        "generation": cycle.evolution.generation,
        "history_items": len(cycle.state.history),
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    learning = cycle.evolve_cycle(cognition, {"cycle": "prepared"})
    ...
    return ChatResponse(
        ...
        cognitive={
            ...
            "action": "done", "reflection": "done", "learning": "done",
            "evolution": "done", "inheritance": "ready",
        },
    )
